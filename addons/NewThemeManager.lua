	...
	for idx, val in decoded do
		...
		elseif typeof(val) == "string" then
			local colorSuccess = pcall(function()
				local color = Color3.fromHex(val)
				if Options[idx] then
...
function ThemeManager:CreateOptions(groupbox)
	...
	groupbox:AddButton("Set as Default", function()
		...
		Library:Notify({
			Title = "Success",
			Description = string.format("Set default theme to %q.", Options.ThemeManager_ThemeList.Value),
			Time = 3,
			Icon = "circle-check"
		})
	end)
	...
	groupbox:AddButton("Create Theme", function() 
		...
		Library:Notify({
			Title = "Success",
			Description = string.format("Created theme %q.", Options.ThemeManager_CustomThemeName.Value),
			Time = 3,
			Icon = "circle-check"
		})
	...
	groupbox:AddButton("Delete", function()
		...
		local success, data = self:ExportTheme()
		if not success then
			return Library:Notify({
				Title = "Error",
		...
		Library:Notify({
			Title = "Success",
			Description = "Exported current theme to clipboard.",
			Time = 3,
			Icon = "circle-check"
		})
	end)
	...
	groupbox:AddButton("Set as Default", function()
		if Options.ThemeManager_CustomThemeList.Value ~= nil and Options.ThemeManager_CustomThemeList.Value ~= "" then
			...
			Library:Notify({
				Title = "Success",
				Description = string.format("Set default theme to %q.", Options.ThemeManager_CustomThemeList.Value),
				Time = 3,
				Icon = "circle-check"
			})
		end
		...
		Library:Notify({
			Title = "Success",
			Description = "Set default theme to none.",
			Time = 3,
			Icon = "circle-check"
		})
		Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
	...
	--// Import theme
	groupbox:AddDivider()
	groupbox:AddInput("ThemeManager_ImportData", { Text = "Import Data:" })
	groupbox:AddButton("Import Theme", function()
		local themeData = Options.ThemeManager_ImportData.Value

		if themeData:gsub(" ", "") == "" then
			return Library:Notify({
				Title = "Warning",
		...
		local success, err = self:ImportTheme(themeData)
		if not success then
			return Library:Notify({
				Title = "Error",
		...
		Library:Notify({
			Title = "Success",
			Description = "Theme imported and applied.",
			Time = 3,
			Icon = "circle-check"
		})
	end)
